...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-14 bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 z-50">

      {/* ── Mobile search overlay ───────────────────────────────────── */}
      {mobileSearchOpen && (
        <div className="md:hidden absolute inset-0 bg-white dark:bg-zinc-950 flex items-center px-3 gap-2 z-10">
          <button
            onClick={() => { setMobileSearchOpen(false); setQuery(''); setSuggestions([]); }}
            className="p-2 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 shrink-0"
          >
            <ArrowLeft size={20} />
          </button>
          <div className="flex flex-1 rounded-full border border-zinc-300 dark:border-zinc-700 overflow-hidden bg-zinc-50 dark:bg-zinc-900 focus-within:border-blue-500">
            <input
              ref={mobileInputRef}
              type="text"
              value={query}
            ...
            />
            {query && (
              <button onClick={() => { setQuery(''); setSuggestions([]); }} className="px-2 text-zinc-400">
                <X size={16} />
              </button>
            )}
          </div>
          <button
            ...
            className="p-2 rounded-full bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 shrink-0"
          >
            <Search size={18} />
          </button>
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute top-14 left-0 right-0 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-700 shadow-xl z-50">
              {suggestions.map((s, i) => (
                <button
                  key={i}
                  onMouseDown={() => handleSearch(s)}
                  className="flex items-center gap-3 w-full px-4 py-3 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800 text-left border-b border-zinc-100 dark:border-zinc-800 last:border-0"
                >
      ...
      <div className="flex items-center h-full px-4 gap-4">
        ...
        <div className="flex items-center gap-1 ml-auto shrink-0">
          ...
          {user ? (
                    ...
                    <div className="py-1">
                      {themeOptions.map(t => (
                        <button key={t.value} onClick={() => setTheme(t.value)}
                          className={`flex items-center gap-3 w-full px-4 py-2 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800 ${theme === t.value ? 'text-red-500' : ''}`}>
            ...
            <Link to="/login"
              className="flex items-center gap-2 px-4 py-1.5 rounded-full border border-red-500 text-red-500 text-sm hover:bg-red-50 dark:hover:bg-red-950 transition-colors font-medium">
